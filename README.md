...
* [`src/amplify_cljs_example/styles.cljs`](src/amplify_cljs_example/styles.cljs): CSS compilation source file (ClojureScript,
[Garden](https://github.com/noprompt/garden))
* [`src/amplify_cljs_example/`](src/amplify_cljs_example/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/amplify_cljs_example/core.cljs): contains the SPA entry point, `init`
* [`test/amplify_cljs_example/`](test/amplify_cljs_example/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
5. Clone this repo and open a terminal in the `amplify_cljs_example` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/amplify_cljs_example/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns amplify-cljs-example.example
  (:require [amplify-cljs-example.config :as config])
